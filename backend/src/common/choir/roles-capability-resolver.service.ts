...
@Injectable()
export class RolesCapabilityResolverService {
  constructor(
  ...
  async resolveGrantsToCapabilities(
    ...
    for (const role of resolved.roles) {
      const bundle = ROLE_ROLES_CAPABILITY_BUNDLES[role];
      if (!bundle) continue;
    ...
    return {
      ...
      capabilities: dedupeScopedCapabilities(caps).filter((c) =>
        (CHOIR_ROLES_CAPABILITY_IDS as readonly string[]).includes(c.id),
      ),
    };
  }
  ...
  canManageCustomRole(auth: ResolvedAuth | undefined): boolean {
    if (!auth) return false;
    return this.can(auth, 'choir.custom_role.manage@choir');
  }

  canManageCommitteeRole(auth: ResolvedAuth | undefined): boolean {
    if (!auth) return false;
  ...
  private mapLegacyPermissionsToCapabilities(
    ...
    for (const perm of permissions) {
      ...
      for (const id of mapped) {
        if ((CHOIR_ROLES_CAPABILITY_IDS as readonly string[]).includes(id)) {
          out.push({ id });
        }
      }
    }
