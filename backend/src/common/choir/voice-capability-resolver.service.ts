...
import { CHOIR_VOICE_CAPABILITY_IDS } from './voice-capability-ids';
import { ROLE_VOICE_CAPABILITY_BUNDLES } from './role-voice-capability-bundles';

@Injectable()
export class VoiceCapabilityResolverService {
  constructor(
  ...
  async resolveGrantsToCapabilities(
    ...
    for (const role of resolved.roles) {
      const bundle = ROLE_VOICE_CAPABILITY_BUNDLES[role];
      if (!bundle) continue;
    ...
    return {
      ...
      capabilities: dedupeScopedCapabilities(caps).filter((c) =>
        (CHOIR_VOICE_CAPABILITY_IDS as readonly string[]).includes(c.id),
      ),
    };
  }
  ...
  canViewVoice(auth: ResolvedAuth | undefined): boolean {
    if (!auth) return false;
    return this.can(auth, 'choir.voice.view@choir');
  }
  ...
  private mapLegacyPermissionsToCapabilities(
    ...
    for (const perm of permissions) {
      ...
      for (const id of mapped) {
        if ((CHOIR_VOICE_CAPABILITY_IDS as readonly string[]).includes(id)) {
          out.push({ id });
        }
      }
    }
