...
import { ROLE_OPS_CAPABILITY_BUNDLES } from './role-ops-capability-bundles';

@Injectable()
export class OpsCapabilityResolverService {
  constructor(
  ...
  async resolveGrantsToCapabilities(
    ...
    for (const role of resolved.roles) {
      const bundle = ROLE_OPS_CAPABILITY_BUNDLES[role];
      if (!bundle) continue;
    ...
    return {
      ...
      capabilities: dedupeScopedCapabilities(caps).filter((c) =>
        (CHOIR_OPS_CAPABILITY_IDS as readonly string[]).includes(c.id),
      ),
    };
  }
  ...
  private mapLegacyPermissionsToCapabilities(
    ...
    for (const perm of permissions) {
      ...
      for (const id of mapped) {
        if ((CHOIR_OPS_CAPABILITY_IDS as readonly string[]).includes(id)) {
          out.push({ id });
        }
      }
    }
