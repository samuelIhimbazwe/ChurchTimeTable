...
import { CHOIR_JOIN_CAPABILITY_IDS } from './join-capability-ids';
import { ROLE_JOIN_CAPABILITY_BUNDLES } from './role-join-capability-bundles';

@Injectable()
export class JoinCapabilityResolverService {
  constructor(
  ...
  async resolveGrantsToCapabilities(
    ...
    for (const role of resolved.roles) {
      const bundle = ROLE_JOIN_CAPABILITY_BUNDLES[role];
      if (!bundle) continue;
    ...
    return {
      ...
      capabilities: dedupeScopedCapabilities(caps).filter((c) =>
        (CHOIR_JOIN_CAPABILITY_IDS as readonly string[]).includes(c.id),
      ),
    };
  }
  ...
  canReviewJoin(auth: ResolvedAuth | undefined): boolean {
    if (!auth) return false;
    return (
      this.can(auth, 'choir.join.review@choir')
  ...
  private mapLegacyPermissionsToCapabilities(
    ...
    for (const perm of permissions) {
      ...
      for (const id of mapped) {
        if ((CHOIR_JOIN_CAPABILITY_IDS as readonly string[]).includes(id)) {
          out.push({ id });
        }
      }
    }
