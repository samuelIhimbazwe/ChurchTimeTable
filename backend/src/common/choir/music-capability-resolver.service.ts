...
import { CHOIR_MUSIC_CAPABILITY_IDS } from './music-capability-ids';
import { ROLE_MUSIC_CAPABILITY_BUNDLES } from './role-music-capability-bundles';

@Injectable()
export class MusicCapabilityResolverService {
  constructor(
  ...
  async resolveGrantsToCapabilities(
    ...
    for (const role of resolved.roles) {
      const bundle = ROLE_MUSIC_CAPABILITY_BUNDLES[role];
      if (!bundle) continue;
    ...
    return {
      ...
      capabilities: dedupeScopedCapabilities(caps).filter((c) =>
        (CHOIR_MUSIC_CAPABILITY_IDS as readonly string[]).includes(c.id),
      ),
    };
  }
  ...
  canViewMusic(auth: ResolvedAuth | undefined): boolean {
    if (!auth) return false;
    return (
      this.can(auth, 'choir.music.view@choir')
      || this.can(auth, 'choir.music.manage@choir')
      || this.canViewRehearsal(auth)
    );
  }

  canManageMusic(auth: ResolvedAuth | undefined): boolean {
    if (!auth) return false;
    return this.can(auth, 'choir.music.manage@choir');
  }

  canViewRehearsal(auth: ResolvedAuth | undefined): boolean {
    if (!auth) return false;
    return (
      this.can(auth, 'choir.rehearsal.view@choir')
      || this.can(auth, 'choir.rehearsal.manage@choir')
      || this.can(auth, 'choir.music.view@choir')
      || this.can(auth, 'choir.music.manage@choir')
    );
  }

  canManageRehearsal(auth: ResolvedAuth | undefined): boolean {
    if (!auth) return false;
    return (
      this.can(auth, 'choir.rehearsal.manage@choir')
      || this.can(auth, 'choir.music.manage@choir')
  ...
  private mapLegacyPermissionsToCapabilities(
    ...
    for (const perm of permissions) {
      ...
      for (const id of mapped) {
        if ((CHOIR_MUSIC_CAPABILITY_IDS as readonly string[]).includes(id)) {
          out.push({ id });
        }
      }
    }
