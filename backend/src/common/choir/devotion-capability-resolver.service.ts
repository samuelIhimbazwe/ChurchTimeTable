...
import { CHOIR_DEVOTION_CAPABILITY_IDS } from './devotion-capability-ids';
import { ROLE_DEVOTION_CAPABILITY_BUNDLES } from './role-devotion-capability-bundles';

@Injectable()
export class DevotionCapabilityResolverService {
  constructor(
  ...
  async resolveGrantsToCapabilities(
    ...
    for (const role of resolved.roles) {
      const bundle = ROLE_DEVOTION_CAPABILITY_BUNDLES[role];
      if (!bundle) continue;
    ...
    return {
      ...
      capabilities: dedupeScopedCapabilities(caps).filter((c) =>
        (CHOIR_DEVOTION_CAPABILITY_IDS as readonly string[]).includes(c.id),
      ),
    };
  }
  ...
  canViewDevotion(auth: ResolvedAuth | undefined): boolean {
    if (!auth) return false;
    return (
      this.can(auth, 'choir.devotion.view@choir')
      || this.can(auth, 'choir.devotion.create@choir')
      || this.can(auth, 'choir.devotion.publish@choir')
      || this.can(auth, 'choir.devotion.manage@choir')
    );
  }

  canCreateDevotion(auth: ResolvedAuth | undefined): boolean {
    if (!auth) return false;
    return (
      this.can(auth, 'choir.devotion.create@choir')
      || this.can(auth, 'choir.devotion.manage@choir')
  ...
  canPublishDevotion(auth: ResolvedAuth | undefined): boolean {
    if (!auth) return false;
    return (
      this.can(auth, 'choir.devotion.publish@choir')
      || this.can(auth, 'choir.devotion.manage@choir')
  ...
  canManageDevotion(auth: ResolvedAuth | undefined): boolean {
    if (!auth) return false;
    return this.can(auth, 'choir.devotion.manage@choir');
  }
  ...
  private mapLegacyPermissionsToCapabilities(
    ...
    for (const perm of permissions) {
      ...
      for (const id of mapped) {
        if ((CHOIR_DEVOTION_CAPABILITY_IDS as readonly string[]).includes(id)) {
          out.push({ id });
        }
      }
    }
