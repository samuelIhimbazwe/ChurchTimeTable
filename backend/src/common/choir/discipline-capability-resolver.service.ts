...
import { CHOIR_DISCIPLINE_CAPABILITY_IDS } from './discipline-capability-ids';
import { ROLE_DISCIPLINE_CAPABILITY_BUNDLES } from './role-discipline-capability-bundles';

@Injectable()
export class DisciplineCapabilityResolverService {
  constructor(
  ...
  async resolveGrantsToCapabilities(
    ...
    for (const role of resolved.roles) {
      const bundle = ROLE_DISCIPLINE_CAPABILITY_BUNDLES[role];
      if (!bundle) continue;
    ...
    return {
      ...
      capabilities: dedupeScopedCapabilities(caps).filter((c) =>
        (CHOIR_DISCIPLINE_CAPABILITY_IDS as readonly string[]).includes(c.id),
      ),
    };
  }
  ...
  canViewChoirDiscipline(auth: ResolvedAuth): boolean {
    return (
      this.can(auth, 'choir.discipline.view@choir')
      || this.can(auth, 'choir.discipline.manage@choir')
      || this.can(auth, 'choir.discipline.review@choir')
    );
  }
  ...
  private mapLegacyPermissionsToCapabilities(
    ...
    for (const perm of permissions) {
      ...
      for (const id of mapped) {
        if (
          (CHOIR_DISCIPLINE_CAPABILITY_IDS as readonly string[]).includes(id)
        ) {
          out.push({ id });
        }
      }
