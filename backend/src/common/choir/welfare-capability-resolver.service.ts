...
import { CHOIR_WELFARE_CAPABILITY_IDS } from './welfare-capability-ids';
import { ROLE_WELFARE_CAPABILITY_BUNDLES } from './role-welfare-capability-bundles';

@Injectable()
export class WelfareCapabilityResolverService {
  constructor(
  ...
  async resolveGrantsToCapabilities(
    ...
    for (const role of resolved.roles) {
      const bundle = ROLE_WELFARE_CAPABILITY_BUNDLES[role];
      if (!bundle) continue;
    ...
    return {
      ...
      capabilities: dedupeScopedCapabilities(caps).filter((c) =>
        (CHOIR_WELFARE_CAPABILITY_IDS as readonly string[]).includes(c.id),
      ),
    };
  }
  ...
  private mapLegacyPermissionsToCapabilities(
    ...
    for (const perm of permissions) {
      ...
      for (const id of mapped) {
        if (
          (CHOIR_WELFARE_CAPABILITY_IDS as readonly string[]).includes(id)
        ) {
          out.push({ id });
        }
      }
