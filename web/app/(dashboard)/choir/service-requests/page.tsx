...
import { choirApi, choirServiceOpsApi, occurrencesApi } from '@/lib/api'
import { toast } from '@/components/shared/Toast'
import { Card, Badge, CapabilityGate, SkeletonCard } from '@/components/shared'
import { formatDate, formatTime } from '@/lib/utils/format'
import { Calendar, Music } from 'lucide-react'

export default function ChoirServiceRequestsPage() {
  ...
  const { data: requests, isLoading } = useQuery({
    queryKey: ['choir-service-requests'],
    queryFn: () => choirServiceOpsApi.listServiceRequests(),
  })

  const { data: occurrences } = useQuery({
    queryKey: ['service-request-occurrences'],
  ...
  const create = useMutation({
    mutationFn: () =>
      choirServiceOpsApi.createServiceRequest({
        occurrenceId,
    ...
    onSuccess: () => {
      ...
      setNotes('')
      setShowForm(false)
      qc.invalidateQueries({ queryKey: ['choir-service-requests'] })
      qc.invalidateQueries({ queryKey: ['choir-service-requests'] })
    },
  ...
  const review = useMutation({
    mutationFn: (args: { id: string; status: 'APPROVED' | 'REJECTED'; assignedChoirId?: string }) =>
      choirServiceOpsApi.reviewServiceRequest(args.id, {
        status: args.status,
    ...
    onSuccess: (_data, vars) => {
      toast.success(
        vars.status === 'APPROVED'
          ? 'Request approved — assign the choir from scheduling'
          : 'Request updated',
      )
      qc.invalidateQueries({ queryKey: ['choir-service-requests'] })
      qc.invalidateQueries({ queryKey: ['choir-service-requests'] })
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="font-display text-3xl text-text-primary">Service requests</h2>
          <p className="text-text-secondary text-sm mt-1">
            Request or review choir assignments for published services
          </p>
        </div>
        <CapabilityGate platformUiCapability="choir-service-request-create">
          <button
            type="button"
          ...
          {requests?.map((r) => (
            <Card key={r.id} padding="md">
              <div className="flex items-start justify-between gap-4">
                ...
                {r.status === 'PENDING' && (
                  <CapabilityGate platformUiCapability="choir-service-request-schedule">
