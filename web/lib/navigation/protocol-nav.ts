import {
  ...
  Calendar, DatabaseBackup, Settings2,
} from 'lucide-react'
...
const PROTOCOL_POSITION_HUB_LINKS: NavItem[] = [
  { label: 'Admin hub',        icon: Settings2,  path: 'admin' },
...
const HUB_PERMISSIONS: Record<string, string[]> = {
  admin: ['committee.member.manage', 'protocol.invite', 'protocol.claim.review'],
  president: ['protocol.oversight', 'protocol.manage'],
  'vice-president': ['protocol.contribution.view.all', 'protocol.finance.view'],
...
function opsItemsForPermissions(permissions: string[]): NavItem[] {
  ...
  if (permissions.includes('protocol.manage')) {
    items.push({ label: 'Admin settings', icon: Settings2, path: protocolPath('admin/settings') })
  }
