...
const HUB_PERMISSIONS: Record<string, string[]> = {
  president: ['protocol.oversight', 'protocol.operational.monitor', 'protocol.manage'],
  coordinator: ['protocol.team.manage', 'protocol.operational.monitor', 'protocol.attendance.manage'],
  treasury: ['protocol.finance.view', 'protocol.finance.manage', 'protocol.finance.approve'],
  secretary: ['event:write', 'protocol.operational.monitor'],
  'team-leader': ['protocol.team.head', 'protocol.attendance.manage'],
}
...
function opsItemsForPermissions(permissions: string[]): NavItem[] {
  ...
  if (permissions.some((p) => ['protocol.team.manage', 'protocol.oversight', 'protocol.manage'].includes(p))) {
    ...
    items.push({ label: 'Generate teams', icon: Calendar, path: protocolPath('teams/generate') })
  }
