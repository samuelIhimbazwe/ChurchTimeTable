/** UI-facing capability IDs — must stay in sync with backend (see music-capability-contract.spec.ts). */
...
export function isMusicUiCapability(uiId: string): boolean {
  return MUSIC_UI_CAPABILITY_REGISTRY.some((d) => d.id === uiId);
}
