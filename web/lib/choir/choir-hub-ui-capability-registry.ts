/** UI-facing capability IDs — must stay in sync with backend (see choir-hub-capability-contract.spec.ts). */
...
export function isChoirHubUiCapability(uiId: string): boolean {
  return CHOIR_HUB_UI_CAPABILITY_REGISTRY.some((d) => d.id === uiId);
}
