/** UI-facing capability IDs — must stay in sync with backend (see devotion-capability-contract.spec.ts). */
...
export function isDevotionUiCapability(uiId: string): boolean {
  return DEVOTION_UI_CAPABILITY_REGISTRY.some((d) => d.id === uiId);
}
