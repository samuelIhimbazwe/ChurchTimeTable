/** UI-facing capability IDs — must stay in sync with backend (see care-hub-capability-contract.spec.ts). */
...
export function isCareHubUiCapability(uiId: string): boolean {
  return CARE_HUB_UI_CAPABILITY_REGISTRY.some((d) => d.id === uiId);
}
