...
class ProtocolContributeScreen extends ConsumerStatefulWidget {
  const ProtocolContributeScreen({super.key});

  @override
  ConsumerState<ProtocolContributeScreen> createState() =>
      _ProtocolContributeScreenState();
}

class _ProtocolContributeScreenState
    extends ConsumerState<ProtocolContributeScreen> {
  ...
  Future<void> _loadOptions() async {
    ...
    try {
      ...
      await api.loadToken();
      final res =
          await api.dio.get('/finance/contributions/protocol/submit-options');
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Submitted to protocol treasurer for confirmation'),
        ),
      );
  ...
  Widget build(BuildContext context) {
    final payment = _options?['payment'] as Map<String, dynamic>?;

    return Scaffold(
      appBar: AppBar(title: const Text('Protocol contribution')),
      body: _error != null
              ...
              : ListView(
                  ...
                  children: [
                    if (payment != null)
                      Card(
                        child: ListTile(
                          title: const Text('Treasury payment'),
                          subtitle: Text(
                            payment['momoNumber']?.toString() ??
                                payment['instructions']?.toString() ??
                                'See protocol treasurer for MoMo details',
                          ),
                        ),
                      ),
                    ...
                    FilledButton(
                      ...
                      child: _submitting
                          ...
                          : const Text('Submit to treasurer'),
                    ),
                  ],
                ),
    );
